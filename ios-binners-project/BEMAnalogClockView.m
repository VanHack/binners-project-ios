...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.enableDigit == YES) {
        ...
        for(unsigned i = 0; i < 12; i ++){
            ...
            NSString* emptySpace = i+1 < 10 ? @" " : @"";
            NSString* hourNumber = @"";
            
            if (_hourOrMinuteSelected == 0) {
                hourNumber = [NSString stringWithFormat:@"%@%u", emptySpace,i+1];
            } else {
                emptySpace = i+1 < 2 ? @" " : @"";
                
                if(i+1 == 12) {
                    emptySpace = @" ";
                    hourNumber = [NSString stringWithFormat:@"%@%u", emptySpace,0];

                } else {
                    hourNumber = [NSString stringWithFormat:@"%@%u", emptySpace,(i+1)*5];
                }
